...
const axios = require("axios");
  ...
  methods: {
    ...
    //=====Popular=========
    getPopular(){
      axios.get("https://share-sport.2m-sport.com/api/popular").then((res)=>{
    ...
    //=====article=========
    getarticle(){
        axios.get("https://share-sport.2m-sport.com/api/article").then((res)=>{
...
<div class="lg:px-0 md:px-0 md:mt-2">
  ...
  <!-- <div class="max-w-6xl mx-auto"> -->
    <div class="flex flex-col gap-x-2 gap-y-2 lg:gap-x-5 lg:gap-y-4 md:flex-row md:gap-x-5 md:gap-y-4">
      <div class=" from-gray-100 lg:md:w-9/12 md:w-9/12 px-2 rounded-md w-full">
        <div class="flex justify-between text-sm lg:pt-3 md:pt-3 lg:pb-3 md:pb-3 pt-2 pb-2 border-b bg-fan-primary"
        style="border-radius:5px 5px 0px 0px;" v-if="livestatus == true"> 
          ...
          <div v-for="(value, key, index) in livestreams" :key="index">
            <div class="box-twon">
                <a :href="'/livestream/'+value.id" class="">
                  <div class="relative">
                    <img v-bind:src="'https://share-sport.2m-sport.com/upload/'+ value.photo" class="radis-sila" ></div>
